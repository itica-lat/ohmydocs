// deps needed: bun add @react-pdf/renderer
import {
  Document,
  Page,
  Text,
...
// --- Style factory ------------------------------------------------------------
// All sizes in pt. 1rem = 12pt (assuming 96dpi baseline).
// Page padding = 72pt ≈ 1 inch (US Letter standard margin).
...
function makeStyles(p: Palette) {
  return StyleSheet.create({
    ...
    // Section (wrap=false keeps heading + lead on same page)
    section: {
      marginTop: 36,
      marginBottom: 18,
    },
...
function ImagePDF({ block, s: _s }: { block: ImageBlock; s: S }) {
  ...
  return (
    <View style={{ marginTop: 10, marginBottom: 14 }}>
      <Image
        src={block.src}
        style={{
          borderRadius: block.bordered ? 4 : 0,
          maxWidth: "100%",
        }}
      />
      {block.caption ? (
...
function BlockPDF({ block, allBlocks, s }: { block: Block; allBlocks: Block[]; s: S }) {
  switch (block.type) {
    ...
    case "image":
      return <ImagePDF block={block} s={s} />;
    case "quote":
...
export async function renderDocumentToPdf(
  ...
  return renderToBuffer(<DocumentPDF doc={doc} branding={branding} />);
}
